...
data class DogBreed(val name: String, val code: String) {
    companion object {
        fun create(species: String, breed: String? = null): DogBreed {
            return DogBreed(
                name = if (breed.isNullOrEmpty()) species.replaceFirstChar { it.uppercaseChar() } else "${breed.replaceFirstChar { it.uppercaseChar() }} ${species.replaceFirstChar { it.uppercaseChar() }}",
